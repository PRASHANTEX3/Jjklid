...
def get_attack_details(message):
    ...
    if is_authorized(user_id):
        try:
            ...
            if len(command) == 3:  # Expecting only 3 parameters now: host, port, time
                target = command[0]
                port = int(command[1])
                ...
                if duration > MAX_ATTACK_DURATION:
                    ...
                    full_command = f"./🚀Attack {target} {port} {duration} --method BGMI-VIP"
...
def approve_user(message):
    if message.from_user.id == YOUR_OWNER_ID or admin_ids or message.from_user.id == co_owner:
